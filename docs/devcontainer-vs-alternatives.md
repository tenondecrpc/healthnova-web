...
**When to reconsider**: If HealthNova later needs local services (e.g., a local DynamoDB for offline testing, or a mock auth server), Docker Compose could be introduced *alongside* the devcontainer — `devcontainer.json` supports a `dockerComposeFile` property that lets both coexist.
